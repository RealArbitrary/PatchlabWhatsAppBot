...
**Non-image media is rejected, not silently dropped.** While `AwaitingPhotos`, anything arriving as `video`, `audio`, `document`, or `sticker` — or an `image`-typed message whose reported `mime_type` isn't `image/jpeg`, `image/png`, or `image/webp` — gets a reply ("Only photos are accepted — please send an image, or wait and we'll continue without one.") and is otherwise a complete no-op: nothing is downloaded, nothing is saved, no `TicketPhotos` row is written, and neither timer is touched. A rejection during the initial wait doesn't start the debounce, and a rejection during the debounce doesn't reset it — from `PhotoWaitCoordinator`'s point of view it's exactly as if nothing had been sent, and whichever timer was already running keeps counting down unaffected.
...
> ✅ **CI/CD — verified working on MULTIPOINT as of September 2, 2026.** The pipeline (`.github/workflows/deploy.yml`, with the `PatchlabNgrokSync` steps already removed — see "Production Infrastructure" below) has now been run for real against MULTIPOINT via [PR #11](https://github.com/RealArbitrary/PatchlabWhatsAppBot/pull/11)'s merge, twice:
> - **First attempt failed** at the `Stop PatchlabWhatsAppBot service` step with `OpenService(): Access is denied` — an OS-level permissions issue: the GitHub Actions runner's service account (the runner itself is now literally named `MULTIPOINT`) didn't have rights to control the `PatchlabWhatsAppBot` Windows service. Not a code/path/YAML problem, and not fixable from this repo — it needed a server-side account fix.
> - **After that was fixed, a re-run (`gh run rerun`) succeeded end-to-end**: stop, clean, publish, start (including the `SERVICE_RUNNING` poll/retry logic — confirmed working, not just unexercised) all passed.
>
> Only the `PatchlabWhatsAppBot` service path has been exercised this way; there's no `PatchlabNgrokSync` step left to verify since it was removed. If the runner's account setup ever changes again, the `OpenService(): Access is denied` failure mode above is the first thing to check.
...
1. **Pick a process manager** to keep the app running as a background service rather than an interactive console app. On Windows, [NSSM](https://nssm.cc/) (currently in use) wraps any executable as a proper Windows service — install, point it at the published `.exe`, set it to auto-start, done. Any equivalent works (a native Windows Service, a systemd unit on Linux, a container orchestrator, etc.) — the app itself doesn't care, it's just `dotnet run`/a published executable underneath.
   - **Gotcha:** NSSM can report `SERVICE_START_PENDING` momentarily on a start command even when the service comes up healthy seconds later. A deploy script that treats any non-`SERVICE_RUNNING` response as a hard failure will false-positive here — prefer a short retry/wait loop over a single immediate status check. `deploy.yml`'s start steps do this (poll `nssm status` until `SERVICE_RUNNING` or a timeout) — confirmed actually working against MULTIPOINT, see the CI/CD note above.
...
- **A self-hosted GitHub Actions runner needs explicit rights to control other Windows services it's not itself.** `nssm stop`/`start PatchlabWhatsAppBot` from a pipeline step can fail with `OpenService(): Access is denied` even though the same command works fine run interactively — the runner's own service account has to be granted control rights on the target service (or run as an account that already has them), it doesn't inherit anything from being "on the same machine." Hit this for real on the first MULTIPOINT deploy — see the CI/CD note in "Deploying to a server".
