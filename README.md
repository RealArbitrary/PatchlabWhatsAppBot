...
> ⚠️ **`sql/seed-local-dev.sql` is a manual, local-only tool. It WIPES and replaces every row in `TicketComments`, `TicketFeedback`, `Tickets`, and `Customers`. Never run it against a production database.** It is data-only (no `CREATE`/`ALTER TABLE`) and is deliberately **not** wired into `Program.cs`'s migrate-on-startup path — it only ever runs when a human deliberately invokes it.
...
It refuses to run unless you pass `-v ConfirmServer` matching the connection's *exact* `@@SERVERNAME` — this forces you to actually look at what server you're about to run it against every single time, rather than accidentally firing it at whatever a SQL client's default connection happens to be:

```powershell
# 1. Find the server name of the instance you're about to connect to:
sqlcmd -S localhost -d Patchlab -E -C -Q "SELECT @@SERVERNAME"

# 2. Only after confirming that's your own local/dev box, run the seed:
sqlcmd -S localhost -d Patchlab -E -C -v ConfirmServer="<paste the exact value from step 1>" -i sql\seed-local-dev.sql
```

If `-v ConfirmServer` is omitted or doesn't match, the script prints a refusal message and exits without touching any data.
